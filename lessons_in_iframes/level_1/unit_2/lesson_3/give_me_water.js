...
let howManyTries;
if (sessionStorage.userHasTriedToWinThisManyTimesAlready) {
  howManyTries = Number(sessionStorage.userHasTriedToWinThisManyTimesAlready) + 1;
  if (howManyTries<6) {    sessionStorage.userHasTriedToWinThisManyTimesAlready = howManyTries.toString();   } // Don't get any easier after the 5th try.
} else {
  howManyTries = 1;
  sessionStorage.userHasTriedToWinThisManyTimesAlready = "1";
}
...
function vocabluaryBoxIsClosed() { // Will fire from within createAndHandleListenManyTimesBox
  // Unblur
  unblurAndGetReady(); // 2500ms to complete focus
  if (sessionStorage.userHasTriedToWinThisManyTimesAlready == "2" || sessionStorage.userHasTriedToWinThisManyTimesAlready == "4" || sessionStorage.userHasTriedToWinThisManyTimesAlready == "5") {
...
function getReadyToPlayTheGameOnMobileF() {
  ...
  /* THESE MUST NOT CREATE CONFLICT WITH THE NAV MENU SWIPE UP-DOWN */ // USE stopPropagation instead of parent.swipeNavMenuIsLocked = true;
  ...
  function detectIfBothThumbsAreTouching1() {    //e.preventDefault(); // Necessary???
    if (leftIsTouching&&rightIsTouching) {
        ...
        //// USE stopPropagation instead of parent.swipeNavMenuIsLocked = true; // CAREFUL: Don't forget to enable it at the end of the game once the conflict is over !!! !!! !!!
...
function whenWinHappens() {
  ...
  //// parent.swipeNavMenuIsLocked = false; // Will be handled by js_for_all_iframed_lesson_htmls
  howManyTries = 1; sessionStorage.userHasTriedToWinThisManyTimesAlready = "1"; // Reset difficulty
  //setTimeout(function () { say5.play(); injectTextIntoTheHelpBoxP.innerHTML = explanationB; },250);
  setTimeout(function () {  parent.ayFreym.src = "/lessons_in_iframes/we_are_working_for_new_levels/index.html";  },5000);
}
